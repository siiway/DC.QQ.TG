...
if [ -d "./publish" ]; then
...
elif [ -d "$default_install_dir/publish" ]; then
    ...
    if [ ! -d "$install_dir/publish" ]; then
        print_color "$RED" "Could not find publish directory in $install_dir"
...
if [ ! -d "$publish_dir" ]; then
    print_color "$YELLOW" "Creating publish directory: $publish_dir"
    mkdir -p "$publish_dir"
fi
